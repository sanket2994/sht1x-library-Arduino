...
float readTempC(int data, int clk)
{
  ...
  obtVal=getTempfrmSensor(data,clk); //get the temperature reading from sensor (raw data) 
...
int getTempfrmSensor(int data, int clk)
{
  pinMode(data,INPUT);
  pinMode(clk,OUTPUT);
  ...
  sendCmd(cmd, data, clk);    //Send command to read temperature from sensor
  
  waitForResult(data, clk); //wait till sensor finishes reading data
  
  int val=ShiftIn(data, clk); //read the data from the Sensor
...
int getHumidityfrmSensor(int data, int clk)
{
  ...
  sendCmd(cmd, data, clk);  // sendcmd to read humidity to sensor
  
  waitForResult(data, clk); //poll untill sensor has finished measuring data 
  
  int val=ShiftIn(data, clk); //read the data from sensor
  return val;
  
} 
  
//Send the command to read temp or humidity to microcontroller  
void sendCmd(int cmd, int data, int clk)
{
  int ack;
  pinMode(data,OUTPUT);
  pinMode(clk,OUTPUT);
...
void waitForResult(int data, int clk) 
{
